...
// Generate cloud of dispersed positions
...
// Generate target positions on cylinder surface
function makeCylinderTargets(count: number): Float32Array {
  const arr = new Float32Array(count * 3);
  for (let i = 0; i < count; i++) {
    const t = i / count;
    const theta = Math.random() * Math.PI * 2;
    // Cylinder body range: y from -1.6 to 1.7
    const y = -1.6 + t * 3.3 + (Math.random() - 0.5) * 0.1;
    // Radius varies with y (approximate lathe shape)
    const r = y < -1.5 ? 0.72 :
              y < 0.8  ? 0.75 :
...
export default function PixelParticles() {
  ...
  useFrame(({ clock }) => {
    ...
    // Color: orange during birth, white after assembled, orange again during final transform
    ...
    // Hide particles when fully assembled (they become the cylinder itself)
